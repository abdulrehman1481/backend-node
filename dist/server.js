...
app.use((0, cors_1.default)({ origin: CORS_ORIGIN === "*" ? true : CORS_ORIGIN.split(",").map((v) => v.trim()) }));
...
function isEligibleToDonate(isAvailable, weightKg, lastDonationDate) {
...
function timingSafeEqualBase64(aBase64, bBase64) {
    try {
...
function makeDjangoPasswordHash(password) {
    const iterations = Number(process.env.DJANGO_PBKDF2_ITERATIONS || 870000);
    const salt = node_crypto_1.default.randomBytes(12).toString("base64url");
    const digest = node_crypto_1.default.pbkdf2Sync(password, salt, iterations, 32, "sha256").toString("base64");
...
async function verifyPassword(password, storedHash) {
    ...
    if (storedHash.startsWith("pbkdf2_sha256$")) {
        ...
        if (!Number.isFinite(iterations) || iterations <= 0)
            return false;
        const computed = node_crypto_1.default.pbkdf2Sync(password, salt, iterations, 32, "sha256").toString("base64");
    ...
    if (storedHash.startsWith("bcrypt_sha256$")) {
        ...
        const preHashed = node_crypto_1.default.createHash("sha256").update(password).digest("hex");
        return bcryptjs_1.default.compare(preHashed, bcryptEncoded);
    }
    if (storedHash.startsWith("bcrypt$")) {
        const bcryptEncoded = storedHash.slice("bcrypt$".length);
        return bcryptjs_1.default.compare(password, bcryptEncoded);
    }
    return bcryptjs_1.default.compare(password, storedHash);
}
function issueAccessToken(userId, role) {
    const payload = { sub: userId.toString(), role, type: "access" };
    return jsonwebtoken_1.default.sign(payload, ACCESS_SECRET, { expiresIn: ACCESS_EXPIRES });
}
function issueRefreshToken(userId, role) {
    const payload = { sub: userId.toString(), role, type: "refresh" };
...
function assertRequestOpen(status) {
    return status === "ACTIVE" || status === "PARTIAL";
}
function requestWithActionCount(item) {
    return {
        id: item.id,
...
async function inferCityFromLocation(lat, lng) {
...
const registerSchema = zod_1.z.object({
    ...
    hospital_center_id: zod_1.z.number().int().positive().optional(),
});
...
app.post("/api/auth/token/", asyncHandler(async (req, res) => {
    ...
    const refresh = issueRefreshToken(user.id, user.role);
    res.json({ access, refresh });
}));
const refreshSchema = zod_1.z.object({ refresh: zod_1.z.string().min(1) });
app.post("/api/auth/token/refresh/", asyncHandler(async (req, res) => {
    const parsed = refreshSchema.safeParse(req.body);
    if (!parsed.success) {
        return void res.status(400).json({ detail: "refresh is required." });
    }
    try {
        const decoded = jsonwebtoken_1.default.verify(parsed.data.refresh, REFRESH_SECRET);
...
app.patch("/api/auth/me/", auth(), asyncHandler(async (req, res) => {
    ...
    if (typeof data.phone_number === "string")
        updateData.phoneNumber = normalizePhone(data.phone_number);
...
app.get("/api/profiles/donor/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
...
app.patch("/api/profiles/donor/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
    const schema = zod_1.z.object({
        ...
        last_donation_date: zod_1.z.string().nullable().optional(),
...
app.get("/api/profiles/hospital/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
...
app.patch("/api/profiles/hospital/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
    const schema = zod_1.z.object({
        facility_name: zod_1.z.string().optional(),
        license_number: zod_1.z.string().optional(),
        nodal_officer_name: zod_1.z.string().optional(),
...
function computeRequestStatus(unitsRequired, unitsFulfilled, requestedStatus) {
    ...
    if (currentUser.role === "HOSPITAL") {
        ...
        if (donorLoc.lat !== null && donorLoc.lng !== null) {
            for (const item of activeRequests) {
                ...
                if (reqLoc.lat === null || reqLoc.lng === null)
                    continue;
        ...
        if (includeHistory) {
            ...
            for (const it of actionIds)
                nearbyIds.add(it.bloodRequestId);
            for (const it of pingIds)
                nearbyIds.add(it.bloodRequestId);
        }
...
app.get("/api/requests/:requestId/", auth(), asyncHandler(async (req, res) => {
...
const createRequestSchema = zod_1.z.object({
    patient_name: zod_1.z.string().min(1),
    description: zod_1.z.string().optional().default(""),
    patient_age: zod_1.z.number().int().positive().optional(),
    blood_group_needed: zod_1.z.string().min(2),
    units_required: zod_1.z.number().int().positive(),
    urgency: zod_1.z.enum(["STANDARD", "URGENT", "CRITICAL"]),
    required_by_datetime: zod_1.z.string().datetime(),
    hospital_name: zod_1.z.string().min(1),
    location: zod_1.z.object({ lat: zod_1.z.number(), lng: zod_1.z.number() }),
});
app.post("/api/requests/create/", auth(), asyncHandler(async (req, res) => {
...
app.patch("/api/requests/:requestId/status/", auth(), asyncHandler(async (req, res) => {
    const requestId = Number(req.params.requestId);
    const schema = zod_1.z
        .object({
        status: zod_1.z.enum(["ACTIVE", "PARTIAL", "FULFILLED", "CLOSED"]).optional(),
        units_fulfilled: zod_1.z.number().int().min(0).optional(),
    })
    ...
    const status = computeRequestStatus(item.unitsRequired, unitsFulfilled, parsed.data.status);
...
app.get("/api/requests/:requestId/actions/", auth(), asyncHandler(async (req, res) => {
    const requestId = Number(req.params.requestId);
...
app.post("/api/requests/:requestId/actions/", auth(), asyncHandler(async (req, res) => {
    const requestId = Number(req.params.requestId);
    const schema = zod_1.z.object({ action_type: zod_1.z.enum(["VOLUNTEER", "FLAG", "SUPPORT"]), note: zod_1.z.string().optional() });
    const parsed = schema.safeParse(req.body);
    if (!parsed.success) {
        return void res.status(400).json(parseZodError(parsed.error));
    }
    ...
    if (parsed.data.action_type === "VOLUNTEER") {
        ...
        if (!donor)
            return void res.status(400).json({ detail: "Donor profile not found." });
...
app.get("/api/requests/:requestId/comments/", auth(), asyncHandler(async (req, res) => {
    const requestId = Number(req.params.requestId);
...
app.post("/api/requests/:requestId/comments/", auth(), asyncHandler(async (req, res) => {
    const requestId = Number(req.params.requestId);
...
app.delete("/api/requests/comments/:commentId/", auth(), asyncHandler(async (req, res) => {
    const commentId = Number(req.params.commentId);
    const comment = await prisma.requestComment.findUnique({ where: { id: commentId } });
    if (!comment)
        return void res.status(404).json({ detail: "Comment not found." });
...
app.post("/api/requests/:requestId/trigger-matching/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
    const requestId = Number(req.params.requestId);
    const item = await prisma.bloodRequest.findUnique({ where: { id: requestId } });
    if (!item)
        return void res.status(404).json({ detail: "Blood request not found." });
...
app.get("/api/donors/radar/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
    const bloodGroup = String(req.query.blood_group || "").trim();
    if (!bloodGroup)
        return void res.status(400).json({ detail: "blood_group query param is required." });
...
app.post("/api/donors/:donorId/ping/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
    const donorId = Number(req.params.donorId);
    const requestId = Number(req.body?.request_id);
    if (!requestId)
        return void res.status(400).json({ request_id: "request_id is required." });
...
app.get("/api/dashboard/donor/inbox/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
    const donor = await prisma.donorProfile.findUnique({ where: { userId: req.user.id } });
    if (!donor)
        return void res.status(400).json({ detail: "Donor profile not found. Complete your profile first." });
...
app.get("/api/dashboard/donor/inbox/history/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
    const donor = await prisma.donorProfile.findUnique({ where: { userId: req.user.id } });
    if (!donor)
        return void res.status(400).json({ detail: "Donor profile not found. Complete your profile first." });
    const logs = await prisma.donorPingLog.findMany({
...
app.get("/api/dashboard/donor/inbox/:pingId/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
    const donor = await prisma.donorProfile.findUnique({ where: { userId: req.user.id } });
    if (!donor)
        return void res.status(400).json({ detail: "Donor profile not found. Complete your profile first." });
    ...
    if (!item)
        return void res.status(404).json({ detail: "Ping not found." });
    res.json({
        id: item.id,
        pinged_at: item.pingedAt.toISOString(),
        response_status: item.responseStatus,
...
app.post("/api/dashboard/donor/inbox/:pingId/respond/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
    const donor = await prisma.donorProfile.findUnique({ where: { userId: req.user.id } });
    if (!donor)
        return void res.status(400).json({ detail: "Donor profile not found. Complete your profile first." });
    const pingId = Number(req.params.pingId);
    const schema = zod_1.z.object({
        response_status: zod_1.z.enum(["ACCEPTED", "DECLINED"]),
...
app.get("/api/requests/:requestId/pings/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
    const requestId = Number(req.params.requestId);
    const bloodRequest = await prisma.bloodRequest.findUnique({ where: { id: requestId } });
    if (!bloodRequest || bloodRequest.requesterId !== req.user.id) {
...
app.get("/api/dashboard/hospital/summary/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
    const now = new Date();
...
app.get("/api/dashboard/donor/feed/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
...
app.get("/api/dashboard/donor/eligibility/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
    const donor = await prisma.donorProfile.findUnique({ where: { userId: req.user.id } });
    if (!donor)
        return void res.status(400).json({ detail: "Donor profile not found. Complete your profile first." });
...
app.get("/api/dashboard/donor/summary/", auth(), requireRole("DONOR"), asyncHandler(async (req, res) => {
    const radiusKm = Number(req.query.radius_km || 10);
    const donor = await prisma.donorProfile.findUnique({ where: { userId: req.user.id } });
    if (!donor)
        return void res.status(400).json({ detail: "Donor profile not found. Complete your profile first." });
    ...
    for (const item of open) {
        ...
        if (loc.lat === null || loc.lng === null)
            continue;
        if (distanceKm(donorLoc.lat, donorLoc.lng, loc.lat, loc.lng) <= radiusKm)
            nearbyOpen += 1;
    }
...
app.get("/api/dashboard/hospital/pings/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
...
app.delete("/api/dashboard/hospital/pings/:pingId/", auth(), requireRole("HOSPITAL"), asyncHandler(async (req, res) => {
    const pingId = Number(req.params.pingId);
    const ping = await prisma.donorPingLog.findFirst({
        where: { id: pingId, bloodRequest: { requesterId: req.user.id } },
    });
    if (!ping)
        return void res.status(404).json({ detail: "Ping not found or not owned by current hospital user." });
...
app.get("/api/directory/medical-centers/", auth(false), asyncHandler(async (req, res) => {
